...
def ping_with_timeout(check_fn, timeout_sec=3.0) -> dict:
    ...
    def run():
        try:
            ...
            result.update({"status": "up", "latency_ms": round(ms, 2)})
            result.pop("error", None)
        except Exception as e:
            result.update({"status": "down", "error": str(e)[:100]})
...
def health_check(db: Session = Depends(get_db)):
    services = {
        "database": ping_with_timeout(lambda: db.execute(text("SELECT 1"))),
        "storage": ping_with_timeout(lambda: StorageClient().ping()),
    ...
    overall = (
        ...
        else "degraded" if any(s == "up" for s in statuses) else "unhealthy"
    )
    return {
        "status": overall,
...
def get_metrics(db: Session = Depends(get_db)):
    ...
    return {
        ...
        "predict_endpoint": {"note": "metrics not yet tracked"},
    }
...
def jobs_summary(db: Session = Depends(get_db)):
    ...
    jobs = db.query(TrainingJob).filter(TrainingJob.started_at >= last_7d).all()
