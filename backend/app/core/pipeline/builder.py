...
def build_pipeline(
    ...
    if not features and not auxiliary_cols:
        raise PipelineBuilderError(
            "No valid feature columns remaining after filtering. Check leakage flags and column roles before training."
        )
    ...
    for p in features:
        ...
        key = (
            p.impute_strategy,
            p.transform_strategy,
            p.name if p.transform_strategy == "tfidf" else None,
        )
    ...
    for (impute_strat, transform_strat, _), cols in groups.items():
        ...
        if steps:
            ...
            pipe = Pipeline(steps)
            step_name = (
                f"pipe_{transform_strat}_{cols[0]}"
                if transform_strat == "tfidf"
                else f"pipe_{impute_strat}_{transform_strat}"
            )
    ...
    return Pipeline(
        [("preprocessor", preprocessor), ("model", DummyClassifier(strategy="prior"))]
    )
