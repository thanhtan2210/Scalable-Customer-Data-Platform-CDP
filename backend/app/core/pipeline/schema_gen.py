...
def generate_schema(
    profiles: List[ColumnProfile], dataset_id: str, target_col: str
) -> Tuple[pa.DataFrameSchema, dict]:
    """
    ...
    for p in profiles:
        if (
            p.inferred_role in [DataRole.ID, DataRole.IGNORE, DataRole.TARGET]
            or p.potential_leakage
        ):
            continue
...
def save_schema(
    schema: pa.DataFrameSchema, metadata: dict, dataset_id: str, target_col: str
) -> Tuple[str, str]:
    """
