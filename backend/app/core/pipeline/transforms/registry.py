...
class WinsorizerTransformer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        self.feature_names_in_ = X_df.columns.astype(str).tolist() if hasattr(X_df, 'columns') else None
        return self
...
class ReshapingTfidfVectorizer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        if isinstance(X, pd.DataFrame):
            ...
            X = X[:, 0]
        return self.vectorizer.transform(X).toarray() # convert sparse to dense for downstream compatibility
...
def get_transformer(strategy: str):
    registry = {
        "standard": Pipeline([("winsorize", WinsorizerTransformer(lower=0.01, upper=0.99)), ("scale", StandardScaler())]),
        "log": FunctionTransformer(func=np.log1p, validate=False),
        "power": PowerTransformer(),
        "ohe": OneHotEncoder(handle_unknown='ignore', sparse_output=False, max_categories=10, min_frequency=0.01),
