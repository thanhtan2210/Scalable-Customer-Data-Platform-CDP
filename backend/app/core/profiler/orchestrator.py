...
from ..config import (
    ENTROPY_LIMIT_LOW, ENTROPY_LIMIT_MED, ENTROPY_LIMIT_HIGH,
    ENTROPY_SCORE_LOW, ENTROPY_SCORE_MED, ENTROPY_SCORE_HIGH
)
from .target_analysis import (
    TargetAnalysis, CandidateTarget, TargetSignals, TargetRole,
    ChurnColumnGroupItem, GroupRole
)
...
def _calculate_correlation(target_col: str, col_name: str, dtype: str, unique_count: int, df: pd.DataFrame) -> float:
...
def detect_target(profiles: list[dict], df: pd.DataFrame) -> TargetAnalysis:
    ...
    if recommended_target:
        ...
        for p in profiles:
            col = p["name"]
            if col == recommended_target or p.get("inferred_role") in [DataRole.ID, DataRole.IGNORE]:
                continue
            corr = _calculate_correlation(recommended_target, col, p.get("inferred_dtype", "object"), p.get("unique_count", 0), df)
            if corr > 0.95:
                temp_leakage[col] = corr
                
        # 1. churn_column_group
        churn_column_group.append(ChurnColumnGroupItem(
        ...
        for rank_idx, cand in enumerate(top_candidates):
            ...
            if col == recommended_target:
                ...
                corr = _calculate_correlation(recommended_target, col, p_col.get("inferred_dtype", "object"), p_col.get("unique_count", 0), df)
                if corr > 0.98:
                    suggested_role = TargetRole.DUPLICATE
                elif col in temp_leakage:
                    suggested_role = TargetRole.LEAKAGE
                    
            candidate_targets.append(CandidateTarget(
                name=col,
                rank=rank_idx + 1,
                score=cand["score"],
...
def check_leakage(target_col: str, profiles: list[dict], df: pd.DataFrame):
    ...
    for p in profiles:
        ...
        corr = _calculate_correlation(target_col, col, p["inferred_dtype"], p["unique_count"], df)
        if corr > 0.95:
...
def run_profiling(df: pd.DataFrame) -> Tuple[list[ColumnProfile], TargetAnalysis]:
    ...
    final_profiles = []
    for p in profiles_dict:
        if p["name"] == target_analysis.composite_target.cpi_column_name if (target_analysis.composite_target and target_analysis.composite_target.cpi_column_name in df.columns) else False:
