...
def route_models(df: pd.DataFrame, confirmed_profiles: List[ColumnProfile]):
    ...
    cat_cardinality = sum(p.unique_count for p in confirmed_profiles if p.inferred_role == DataRole.CATEGORICAL)
    is_sparse_prone = has_text or (cat_cardinality > n_rows * 0.1) # Rất nhiều categories so với số dòng
    ...
    if is_sparse_prone or n_rows < 500:
        models.append({
            ...
            "kwargs": {"max_iter": 2000, "solver": "saga"}, # Saga tốt cho l1/l2 trên dữ liệu thưa
            "search_space": {
