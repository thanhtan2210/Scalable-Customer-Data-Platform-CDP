...
graph TD
    ...
    H3 -->|Gate: 6/6 Pass| FINAL[FINAL GATE: Full Suite Verification]
```
...
* [x] **Task 0.1**: Identify reasons for test failure in `predict/batch` (missing synthesized `cpi_score`).
* [x] **Task 0.2**: Fix E2E test scripts to dynamically capture upload metadata (`r2_path`).
* [x] **Task 0.3**: Refactor backend prediction router to dynamically compute missing composite target score.
* [x] **Gate**: 7/7 E2E tests pass.
