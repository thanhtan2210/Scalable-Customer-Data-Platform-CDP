...
- This is an **offline, backtest-style** estimate. It uses a simple feature set and a Logistic Regression model trained during the run. The high numbers (precision@k=1.0, AUC) indicate this small dataset and chosen features are highly separable — results may be optimistic.
- The outreach success rate (`0.3`) and `cost_per_outreach` (`$5`) are assumptions; adjust them to your business values for realistic ROI.
- To prove causal business impact, run a randomized A/B test in production (or a shadow deployment) with tracked assignments. Use the evaluation script and the experiment design notes in `docs` as starting points.

Artefacts generated by the run:

- `reports/offline_evaluation.json` — machine-readable summary of metrics and ROI estimates.
- `reports/offline_evaluation.txt` — human-readable summary.
- `scripts/evaluate_offline.py` — script used to run the offline evaluation (train/load model, compute AUC, precision@k, ROI estimate).

Next steps to strengthen the proof:

- Expand feature engineering and cross-validated model selection (`src/models`), then re-run offline evaluation.
- Calibrate outreach experiments: run an A/B test with statistically powered sample size and track churn and revenue differences.
- Build dashboards (Metabase/Grafana) and log MLflow runs for reproducibility.
...
- Assign customers to groups (deterministic stable split):
...
- Compute required sample size for a target MDE (minimum detectable effect):

```powershell
python scripts/ab_sample_size.py --p0 0.03 --mde 0.006 --alpha 0.05 --power 0.8
```

- After running the campaign, produce an outcomes CSV with columns `CustomerID,churn` (0/1) and analyze:
...
The `analyze_ab_results.py` script computes churn rates per group and a z-test for difference of proportions; it saves a JSON report with `A_rate`, `B_rate`, `delta`, and `p_value`.

Notes:
- Use the deterministic assignment script to ensure stable group membership across systems.
- For production, implement assignment in the production identity store (hashed assignment) and log assignments and exposures to a central events store.
- The sample size script uses proportions (Normal approximation) — validate assumptions for very small rates.
