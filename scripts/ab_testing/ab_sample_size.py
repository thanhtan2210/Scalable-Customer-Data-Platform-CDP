...
def calc(
    p0: float, mde: float, alpha: float = 0.05, power: float = 0.8, ratio: float = 1.0
):
    ...
    n1 = power_tool.solve_power(effect_size=h, power=power, alpha=alpha, ratio=ratio)
    n2 = n1 * ratio
...
def cli():
    ...
    p.add_argument(
        "--p0", type=float, required=True, help="baseline proportion (e.g., 0.03)"
    )
    p.add_argument(
