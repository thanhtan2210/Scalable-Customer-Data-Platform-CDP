...
def analyze(assign_path: Path, outcomes_path: Path):
    ...
    df = (
        a.merge(o, on=["CustomerID", "customerID"][0:1], how="left")
        if False
        else a.merge(o, left_on="CustomerID", right_on="CustomerID", how="left")
    )
    # ensure churn column
    churn_col = (
        "churn"
        if "churn" in df.columns
        else ("Churn" if "Churn" in df.columns else "churn")
    )
    if churn_col not in df.columns:
        raise KeyError("Outcome column churn or Churn not found in outcomes CSV")
