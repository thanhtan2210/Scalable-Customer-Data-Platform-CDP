...
def setup_windows_env(base_dir):
    ...
    try:
        if os.path.exists(adoptium_dir):
            jdk_name = [f for f in os.listdir(adoptium_dir) if f.startswith("jdk-11")][
                0
            ]
...
def clean_dataframe(df):
    ...
    df = df.select(
        [col(c).alias(c.strip().lower().replace(" ", "")) for c in df.columns]
    )
    cols = df.columns
    ...
    if "churnvalue" in cols:
        ...
        df = df.withColumn(
            "Churn", when(lower(col("churnlabel")) == "yes", 1).otherwise(0)
        )
    elif "churn" in cols:
        df = df.withColumn("Churn", when(col("churn").isin("Yes", "1"), 1).otherwise(0))

    df = df.fillna(0, subset=["Churn"])

    # C. Cast numeric columns
    numeric_cols = {"totalcharges": "TotalCharges", "monthlycharges": "MonthlyCharges"}
    for src, dest in numeric_cols.items():
        if src in cols:
            df = df.withColumn(dest, col(src).cast(DoubleType())).fillna(
                0.0, subset=[dest]
            )
    ...
    tenure_col = (
        "tenuremonths"
        if "tenuremonths" in cols
        else ("tenure" if "tenure" in cols else None)
    )
    if tenure_col:
            ...
            schema = pa.DataFrameSchema(
                {
                    "MonthlyCharges": pa.Column(
                        float, checks=pa.Check.ge(0), required=False
                    ),
                    "Churn": pa.Column(
                        int, checks=pa.Check.isin([0, 1]), required=False
                    ),
                }
            )
...
def run():
    ...
    spark = (
        SparkSession.builder.appName("CDP_Telco_ETL")
        .master("local[*]")
