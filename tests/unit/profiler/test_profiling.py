...
def test_target_detection_domain_agnostic():
    ...
    assert suggested == "outcome_bin", f"Không tự động nhận diện được outcome_bin làm target. Nhận diện nhầm: {suggested}"
    ...
    assert suggested_keyword == "binary_col", f"Bị đánh lừa bởi keyword match! Nhận diện: {suggested_keyword}"
...
def test_orchestrator_output_contract():
    ...
    # Kiểm tra suggested_target là string
    assert isinstance(suggested_target, str)
    assert suggested_target == "target_col"
